...
max_epsilon_list = [1e-4, 1e-6, 1e-8]
link_cap_multiplier_list = [1, 100.0, 1000.0]
        ...
        for tm_model in TM_MODEL_LIST:
            ...
            for selected_fidx in fnames_idx:
                ...
                for mcf_method in mcf_grb_method:
                    for link_cap_multiplier in link_cap_multiplier_list:
                        for max_epsilon in max_epsilon_list:
                            ...
                            log_txt = f"{mcf_method},{link_cap_multiplier},{max_epsilon},{file_name}," \
                                      f"{num_paths},{gb_total_flow},{gb_duration},{per_flow_log_file_name}\n"
                            utils.write_to_file(log_txt, log_dir, log_file)
                            print("result:", log_txt)
                            for key, time_s in gb_run_time_dict.items():
